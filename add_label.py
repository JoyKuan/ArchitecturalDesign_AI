...
file_path = os.path.join(os.getcwd(), 'garmin')
files = os.listdir(file_path)

def add_label(data_df, laps_data_df, begin_time, current_date):
    combine = list()
...
for file in files:
    ...
    garmin_person_df = add_label(df, laps_df, start_time, date_)

    write_path = os.path.join(os.getcwd(), 'final', 'garmin_label')
