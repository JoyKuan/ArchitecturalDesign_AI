...
def embrace_add_label(data_df, laps_data_df, begin_time, current_date, fea):
    combine = list()
    count = 0
...
for file in files:
    ...
    embrace_person_df = embrace_add_label(df, laps_df, start_time, date_, col_name)

    write_path = os.path.join(os.getcwd(), 'final', 'embrace_label')
    write_filename = write_path + '/' + file
    embrace_person_df.to_csv(write_filename, index=False)
